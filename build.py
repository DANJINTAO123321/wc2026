...
from predictions import PREDICTIONS, WINNER_ODDS, FINAL_COMBO_ODDS
...
/* === 冠军赔率 === */
.odds-table { width:100%; border-collapse:collapse; font-size:12px; margin-top:6px; }
.odds-table th { background:rgba(255,180,0,0.1); color:#ffb700; font-weight:600; padding:6px 5px; text-align:center; border-bottom:2px solid rgba(255,180,0,0.3); font-size:10px; }
.odds-table td { padding:5px; text-align:center; border-bottom:1px solid rgba(255,255,255,0.06); }
.odds-table tr:hover td { background:rgba(255,180,0,0.04); }
.odds-table .t { text-align:left; }
.odds-table .top3 td { color:#ffb700; font-weight:700; }
.odds-sup { font-size:10px; color:#8aa3bd; margin-bottom:8px; }

.combo-card { background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.08); border-radius:10px; padding:10px; display:flex; justify-content:space-between; align-items:center; margin-bottom:6px; }
.combo-card .teams { font-size:13px; font-weight:600; }
.combo-card .odds { font-size:18px; font-weight:800; color:#ffb700; }
.combo-card .desc { font-size:10px; color:#8aa3bd; }
...
<div class="nav">
  ...
  <button onclick="goTo('odds')">冠军赔率</button>
...
# 4.5 冠军赔率
out += '<div class="section" id="odds"><div class="section-title">🏆 世界杯冠军赔率 (中国体彩 6/2 开售)</div>'
out += '<div class="odds-sup">赔率来源: 中国体育彩票官方 · 隐含概率 = 1/赔率 × 100% · 高盛 AI = Goldman Sachs 蒙特卡洛 5 万次模拟</div>'
out += '<table class="odds-table"><thead><tr><th>排名</th><th style="text-align:left">球队</th><th>1赔X</th><th>隐含概率</th><th>高盛AI</th><th>6/2排名</th></tr></thead><tbody>'
rank = 0
for en, od in WINNER_ODDS.items():
    rank += 1
    t = TEAMS[en]
    odds_str = f"{od['odds']}" if od.get('odds') else '-'
    imp_str = f"{od['implied']}%" if od.get('implied') else '-'
    gs_str = f"{od.get('gs_prob', '-')}%" if od.get('gs_prob') else '-'
    r2_str = f"#{od['rank_jun2']}" if od.get('rank_jun2') else '-'
    tr_class = 'top3' if rank <= 3 else ''
    out += f'<tr class="{tr_class}"><td>{rank}</td><td class="t">{t["zh"]} <span style="color:#8aa3bd;font-size:10px">({en})</span></td><td><b>{odds_str}</b></td><td>{imp_str}</td><td>{gs_str}</td><td>{r2_str}</td></tr>'
out += '</tbody></table>'
out += '</div>'

# 4.6 决赛组合赔率
out += '<div class="section" id="combo"><div class="section-title">🔗 决赛对阵组合赔率</div>'
out += '<div class="odds-sup">预测哪两支球队会师决赛 (中国体彩官方赔率)</div>'
for (teams, od, desc) in FINAL_COMBO_ODDS:
    out += f'<div class="combo-card"><div class="teams">{teams}</div><div class="odds">1赔{od}</div><div class="desc">{desc}</div></div>'
out += '</div>'
